...
const addComments = (contents: string): string => {
  const autogeneratedHash = `${SYNCHRONIZATION_HASH_BASE} ${sha256(contents)}`;
  return `${AUTO_GENERATED_INTRO}\n${autogeneratedHash}\n\n${contents}`;
};

const addAutogeneratedComments = (contents: string): string => {
  if (syncHashExists(contents)) {
    return cleanContent(contents);
  }
  return addComments(contents);
};
